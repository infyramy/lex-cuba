...
const { isDirty, resetDirty } = useFormDirty(profileForm);
...
async function saveProfile() {
  if (!validateAll({ name: profileForm.value.name, email: profileForm.value.email })) return;
    ...
    <div class="mx-auto max-w-3xl space-y-6">
      <div class="admin-hero">
        <div class="space-y-2">
          ...
          <router-link
            to="/admin/users"
            class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition-colors hover:bg-slate-50 hover:text-slate-800"
            v-if="!isSelf"
          >
          ...
          <p class="page-subtitle">Update account identity, access level, and credential controls in a cleaner administrative editor.</p>
        </div>
      </div>

      <LoadingSkeleton v-if="loading" variant="form" :lines="5" />
          ...
          <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div class="p-4">
              ...
              <div class="grid gap-3 md:grid-cols-2">
                ...
                <!-- Admin-only fields -->
                <FormField v-if="!isSelf || isNew" label="Role" help-text="Determines which admin features this user can access">
                  <select
                ...
                <FormField v-if="!isSelf || isNew" label="Active">
