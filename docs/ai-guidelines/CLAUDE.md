...
> Source: [Claude Code Blueprint](https://github.com/faizkhairi/claude-code-blueprint)
...
| API / server route | curl or fetch the live endpoint, check the response |
| Config change | Confirm the config was actually picked up |
| Git operation | `git status` + `git log` to confirm clean state |
| File edit | Re-read the file after editing to confirm the change landed |
| Fact/data update | Search for the NEW value (confirm present) AND OLD value (confirm absent everywhere) |

- Don't assume it worked — run the check.
- Never say "done" without having verified the result.
- Return to plan after interruptions — check todo list and resume in-progress items.
- Finish the current task before expanding scope — note adjacent issues for later, don't detour.

### Diagnose-First Rule (Before Any Fix)

Before writing any fix, always run these checks:

1. `git status` / `git log --oneline -5` — confirm the issue isn't an unstaged deletion or already fixed
2. Identify error source — VSCode Problems panel vs CLI vs runtime logs (don't treat editor diagnostics as CLI errors)
3. Check for existing suppression settings in `.vscode/settings.json` or recent commits
4. Apply minimum viable diagnosis — simplest explanation first

### Plan-First Rule (MANDATORY)

ALWAYS enter plan mode before making any non-trivial changes (modifies >1 file, adds functionality, changes behavior, touches configuration).

**Trivial** (skip plan): single-line typo fix, adding a log, renaming a variable in one file.

Workflow: Plan → User Review → Execute.

### Verify-Before-Exit-Plan

Before exiting plan mode, mechanically check:

1. **Count check** — do file/step counts match?
2. **Path check** — do all referenced paths exist? Grep for OLD values to catch stale copies.
3. **Wiring check** — for every new file, who imports/consumes it?
4. **Policy check** — does the plan contradict any rule in this CLAUDE.md?
5. **Completeness check** — trace each item: creation → wiring → testing.

### Compact Instructions

When compacting, preserve:
- Current todo list with item status
